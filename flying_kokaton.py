...
def main():
    ...
    while True:
        ...
        kt_diff_x=0                                 #こうかとんの移動状態を記憶する変数を用意
        kt_diff_y=0                                 
        
        key_lst=pg.key.get_pressed()                #全キーの押下状態を確認する
        if key_lst[pg.K_UP]:                        #上キー押下
            kt_diff_y=-1                            #Y方向の変数を-1する
        
        elif key_lst[pg.K_DOWN]:                    #下キー押下
            kt_diff_y=1                             #Y方向の変数を+1する
        
        elif key_lst[pg.K_RIGHT]:                   #右キー押下
            kt_diff_x=3                             #X方向の変数を+3する
        
        elif key_lst[pg.K_LEFT]:                    #左キー押下
            kt_diff_x=-1                            #X方向の変数を-1する
        
        kt_rct.move_ip( -1+kt_diff_x , kt_diff_y )  #変数に合わせて移動させる
